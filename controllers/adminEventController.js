...
exports.postCreateEvent = async (req, res) => {
  try {
    ...
    if (startDate === minDateStr && startTime < currentTime) {
      req.flash(
        "error",
        "Start time cannot be in the past for events starting today"
      );
      return res.redirect("/admin/events/create");
    }
    ...
    if (req.file) {
      ...
      eventData.imagePath =
        "https://www.cvent.com/sites/default/files/styles/column_content_width/public/image/2023-11/53322146052_90bc13d238_c.jpg.webp?itok=YayCFh_V";
    }
...
exports.postEditEvent = async (req, res) => {
  try {
    ...
    try {
      ...
      const userEmails = [
        ...new Set(confirmedBookings.map((b) => b.userEmail)),
      ];
...
exports.deleteEvent = async (req, res) => {
  try {
    ...
    // ── Notify all users with confirmed bookings BEFORE deleting ──
    try {
      const confirmedEmails = await Booking.find({
        eventId: event._id,
      ...
      if (confirmedEmails.length > 0) {
        const notifPayload = {
          ...
          eventName: eventName,
          eventDate: eventDate,
          message: `${eventName} on ${eventDate} has been cancelled.`,
          isRead: false,
          createdAt: new Date(),
        };
        await User.updateMany(
          { email: { $in: confirmedEmails } },
          { $push: { notifications: { $each: [notifPayload], $position: 0 } } }
        );
      }
    } catch (notifErr) {
      console.error("Failed to send event_cancelled notifications:", notifErr);
    }

    // ── Cancel all active bookings ──
    await Booking.updateMany(
      { eventId: event._id, status: { $nin: ["cancelled", "expired"] } },
    ...
    req.flash("success", "Event deleted and all bookings cancelled.");
    res.redirect("/admin/dashboard");
  } catch (err) {
...
exports.exportBookingsCsv = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find({ eventId: req.params.id }).sort({
      createdAt: -1,
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${event.eventName.replace(
        /[^a-zA-Z0-9]/g,
        "_"
      )}_attendees.csv"`
    );
