...
exports.cancelBookingById = async (req, res) => {
    try {
        ...
        if (isFullCancel) {
            ...
            // Keep a history entry for cancelled tickets so they appear under "Cancelled" bookings.
            // This creates a separate cancelled booking record for the cancelled quantity.
            ...
            try {
                await bookingModel.create({
                    ...
                    // bookingRef has a unique DB index; keep it unique for the cancellation record.
                    bookingRef: cancellationRef,
                    status: "cancelled"
                });
            } catch (err) {
                // If history insert fails, still keep the main booking consistent.
