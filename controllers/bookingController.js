...
exports.createBooking = async (req, res) => {
    try {
        const { eventId, email, ticketCount: ticketCountRaw } = req.body;
        ...
        if (ticketCount > 5) {
            return res.status(400).json({ success: false, message: 'You can book a maximum of 5 tickets per purchase.' });
        }
        ...
        if (booking) {
            ...
            booking.userName = (booking.userName || user.username || '').trim() || user.username;
    ...
    try {
        ...
        bookings = bookings
            ...
            .map(booking => {
                ...
                // Normalize event title fields across legacy/seed/admin data
                if (booking.eventId) {
                    booking.eventId.eventName = booking.eventId.eventName || booking.eventId.title || booking.eventId.name;
                    booking.eventId.title = booking.eventId.title || booking.eventId.eventName || booking.eventId.name;
                    booking.eventId.name = booking.eventId.name || booking.eventId.eventName || booking.eventId.title;
                }
...
exports.getManageBooking = async (req, res) => {
    try {
        ...
        // Normalize event title fields for display
        booking.eventId.eventName = booking.eventId.eventName || booking.eventId.title || booking.eventId.name;
        booking.eventId.title = booking.eventId.title || booking.eventId.eventName || booking.eventId.name;
        booking.eventId.name = booking.eventId.name || booking.eventId.eventName || booking.eventId.title;
